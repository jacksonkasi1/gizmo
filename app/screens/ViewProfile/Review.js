...
const Review = ({ data, id, name, screenName,route }) => {
    ...
    <View>
      {screenName !== undefined ?
        <>
          ...
          {index === 0 && <Recent id={id} data={data} status={'Recent'} name={name} />}
          {index === 1 && <Popular id={id} data={data} status={'Popular'} name={name} />}
          {index === 2 && <Oldest id={id} data={data} status={'Oldest'} name={name} />}
        </>
        :
        <View>
          <FlatList
            ...
            renderItem={({ item }) => (
              <VideoFlatlist data={item} section={'Review'} />
            )}
